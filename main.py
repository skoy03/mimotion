...
def main(_user, _passwd, _step):
    ...
    if _step == '':
        print("已设置为随机步数（10000-30000）")
        _step = str(random.randint(10000, 30000))
...
def push_server(_sckey, desp=""):
    ...
    if _sckey == '':
        ...
        if json_data['code'] == 0:
            ...
            print(f"{now}\n推送失败：{json_data['message']}")


def push_qmsg(qkey, msg="", qq=""):
    """
    推送消息到QQ
    """
    if qkey == '':
        print("[注意] 未提供qkey，不进行QQ推送！")
    else:
        qmsg_url = f"https://qmsg.zendee.cn/send/{qkey}"
        params = {
            "qq": f"{qq}",
            "msg": msg
        }
        
        response = requests.post(qmsg_url, params=params)
        json_data = response.json()

        if json_data['code'] == 0:
            print(f"推送时间：{now} 推送成功。")
        else:
            print(f"推送时间：{now} 推送失败：{json_data['reason']}")

def push_pushplus(token, title="小米运动助手", template="txt", content=""):
    """
    推送消息到pushplus
    """
    if token == '':
...
class ToPush:
    ...
    def to_push_qmsg(self):
        """
        推送qmsg酱接口
        """
        if str(self.pkey) == '0':
            self.pkey = ''
        push_qmsg(self.pkey, self.push_msg)
